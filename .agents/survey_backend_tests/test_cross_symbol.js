...
async function main() {
  ...
  try {
    // Primary symbol is XAUUSD., but input is AAPL
    const pine = new p.PineTS(bars, 'XAUUSD.', 'D');
    const ctx = await pine.run(ind);
    console.log('Cross-symbol request.security success!');
    ...
    console.error('Cross-symbol request.security failed:', err);
  }
}
