...
import { INDICATOR_PROPS, STRATEGY_PROPS, LIBRARY_PROPS } from './propsSchema';
import type { IPineProp } from './types';

/**
 * Detect whether a script is a `strategy()`, `indicator()`, or `library()`, and extract
 ...
 *                    callee Identifier matching 'indicator' / 'strategy' / 'library'
 *
...
function scanPineDeclaration(source: string): ScannedDeclaration {
    ...
    walk(parsed.ast, (node) => {
        ...
        if (callee.name !== 'indicator' && callee.name !== 'strategy' && callee.name !== 'library') return;
        found = {
            type: callee.name as any,
            args: decodeArgs(callExpr.arguments ?? [], schemaFor(callee.name as any)),
        };
    });
...
function scanJsDeclaration(fn: Function): ScannedDeclaration {
    ...
    walk(parsed, (node) => {
        ...
        found = {
            type: callee.name as any,
            args: decodeArgs(node.arguments ?? [], schemaFor(callee.name as any)),
        };
    });
    return found ?? { type: null, args: {} };
}
...
function schemaFor(type: 'indicator' | 'strategy' | 'library'): IPineProp[] {
    if (type === 'strategy') return STRATEGY_PROPS;
    if (type === 'library') return LIBRARY_PROPS;
    return INDICATOR_PROPS;
}
