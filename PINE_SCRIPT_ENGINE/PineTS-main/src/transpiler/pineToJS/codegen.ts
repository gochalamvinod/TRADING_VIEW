...
export class CodeGenerator {
    ...
    generateVariableDeclaration(node) {
        ...
        // PineScript tuple destructuring => JavaScript const
        const kind = node.kind === 'var' || node.kind === 'varip' ? 'var' : node.kind === 'const' ? 'const' : 'let';
        ...
        for (let i = 0; i < node.declarations.length; i++) {
            ...
            } else if (decl.id?.type === 'ArrayPattern' && Array.isArray(decl.id.elements)) {
                for (const el of decl.id.elements) {
                    const elType = el?.varType;
                    const elName = el?.name;
                    if (elName && typeof elType === 'string') {
                        if (/^[A-Za-z_$][\w$]*$/.test(elType) && elType[0] === elType[0].toUpperCase()) {
                            this.write(this.indentStr.repeat(this.indent));
                            this.write(`"__pineUdtVar:${elName}=${elType}";\n`);
                        } else if (elType.length > 0) {
                            this.write(this.indentStr.repeat(this.indent));
                            this.write(`${JSON.stringify(`__pineTypedVar:${elName}=${elType}`)};\n`);
                        }
                    }
                }
            }
    ...
    // Generate ImportDeclaration
    generateImportDeclaration(node) {
        this.write(this.indentStr.repeat(this.indent));
        const alias = (typeof node.alias === 'object' && node.alias !== null) ? node.alias.name : node.alias;
        if (alias) {
            this.write(`const ${alias} = (typeof ${alias} !== 'undefined' ? ${alias} : (typeof ta !== 'undefined' ? ta : {})); // import ${node.path} as ${alias}\n`);
        } else {
            this.write(`// import ${node.path};\n`);
        }
    }
