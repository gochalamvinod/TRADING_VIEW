...
export class Parser {
    ...
    skipNewlines(allowIndent = false) {
        ...
        // When allowIndent is true, we allow line continuation, but an unindented
        // line (startCol <= 1) is never a continuation line in Pine Script.
        while (this.match(TokenType.NEWLINE) || this.match(TokenType.COMMENT)) {
            if (this.match(TokenType.NEWLINE)) {
                let look = 1;
                while (
                    this.peek(look).type === TokenType.NEWLINE ||
                    this.peek(look).type === TokenType.COMMENT
                ) {
                    look++;
                }
                const nextTok = this.peek(look);
                if (nextTok.type !== TokenType.EOF && nextTok.type !== TokenType.INDENT) {
                    const tokenLen = nextTok.raw ? String(nextTok.raw).length : (nextTok.value !== null && nextTok.value !== undefined ? String(nextTok.value).length : 1);
                    const startCol = nextTok.column - tokenLen;
                    if (startCol <= 1) {
                        break;
                    }
                }
    ...
    parseStatement(handleCommas = true) {
        ...
        if (this.match(TokenType.KEYWORD, 'export')) {
            ...
            if (this.match(TokenType.KEYWORD, 'method')) {
                ...
                throw new Error(`Expected function or method declaration after 'export' at ${this.peek().line}:${this.peek().column}`);
            }
            if (stmt) {
                stmt.isExported = true;
            }
        }
        // Import declaration (libraries)
        else if (this.match(TokenType.KEYWORD, 'import')) {
            stmt = this.parseImportDeclaration();
        }
        // Enum definition
        else if (this.match(TokenType.KEYWORD, 'enum')) {
    ...
    parseImportDeclaration() {
        ...
        while (!this.match(TokenType.NEWLINE) && !this.match(TokenType.SEMICOLON) && !this.match(TokenType.COMMENT) && !this.match(TokenType.EOF)) {
            const tok = this.peek();
            if ((tok.type === TokenType.IDENTIFIER || tok.type === TokenType.KEYWORD) && tok.value === 'as') {
                this.advance(); // consume 'as'
                this.skipNewlines();
                alias = this.expectIdentifierOrContextual().value;
                break;
            }
            path += (tok.raw !== null && tok.raw !== undefined ? tok.raw : tok.value);
            this.advance();
        }
        ...
        return new ImportDeclaration(path, alias);
    }
    ...
    parseVarDeclaration() {
        ...
        // Tuple destructuring: var [a, b] = ... or var [int a, float b] = ...
        if (this.isTupleDestructuring()) {
            return this.parseTupleDestructuring(kind === 'var' ? VariableDeclarationKind.VAR : VariableDeclarationKind.VARIP);
        }
    ...
    isTupleDestructuring() {
        ...
        while (true) {
            ...
            // Check if empty bracket or closing bracket
            ...
            // Each element must start with an identifier
            ...
            while (true) {
                ...
                if (tok.type === TokenType.IDENTIFIER) {
                    hasName = true;
                    i++;
                    // Check if followed by dotted type: . IDENTIFIER
                    while (this.peek(i).type === TokenType.DOT && this.peek(i + 1).type === TokenType.IDENTIFIER) {
                        i += 2;
                    }
                    // Check if followed by array brackets: [ ]
                    if (this.peek(i).type === TokenType.LBRACKET && this.peek(i + 1).type === TokenType.RBRACKET) {
                        i += 2;
                    }
                    // Check if followed by generic brackets: < ... >
                    else if (this.peek(i).type === TokenType.OPERATOR && this.peek(i).value === '<') {
                        i++; // skip <
                        let depth = 1;
                        while (depth > 0 && this.peek(i).type !== TokenType.EOF) {
                            if (this.peek(i).type === TokenType.OPERATOR && this.peek(i).value === '<') depth++;
                            else if (this.peek(i).type === TokenType.OPERATOR && this.peek(i).value === '>') depth--;
                            i++;
                        }
                    }
                    continue;
    ...
    private isMultipleIdentifiersBeforeDelimiter(): boolean {
        let i = 0;
        while (this.peek(i).type === TokenType.NEWLINE) i++;
        if (this.peek(i).type !== TokenType.IDENTIFIER) return false;
        i++; // past 1st identifier
        while (this.peek(i).type === TokenType.NEWLINE) i++;
    ...
    parseTupleElement() {
        ...
        // Check for generic type: type<...> name
            ...
            while (depth > 0 && !this.match(TokenType.EOF)) {
                if (this.match(TokenType.OPERATOR, '<')) {
                    ...
                    varType += this.advance().value;
                    while (this.match(TokenType.DOT) && this.peek().type === TokenType.IDENTIFIER) {
                        varType += '.' + this.advance().value;
                    }
                } else if (this.match(TokenType.COMMA)) {
                    varType += ', ';
        ...
        // Check for dotted type: ns.type name or ns.type[] name
        else if (
            this.peek().type === TokenType.IDENTIFIER &&
        ...
        // Check if there are multiple identifiers before COMMA or RBRACKET:
        // e.g. `int a`, `float b`, `series float a`, `simple int b`
        else if (
            this.peek().type === TokenType.IDENTIFIER &&
            this.isMultipleIdentifiersBeforeDelimiter()
        ) {
            const parts: string[] = [];
            while (this.isMultipleIdentifiersBeforeDelimiter()) {
    ...
    parseTupleDestructuring(kind = VariableDeclarationKind.CONST) {
        ...
        while (!this.match(TokenType.RBRACKET)) {
            ...
            const elem = this.parseTupleElement();
            elements.push(elem);
        ...
        return new VariableDeclaration([new VariableDeclarator(new ArrayPattern(elements), init)], kind);
    }
