...
export class Lexer {
    ...
    error(message: string, line = this.line, col = this.column): never {
        const err: any = new Error(`${message} at ${line}:${col}`);
        err.line = line;
        err.column = col;
        throw err;
    }
    ...
    tokenize() {
        while (this.pos < this.source.length) {
            ...
            this.error(`Unexpected character '${ch}'`, this.line, this.column);
        }
    ...
    handleIndentation() {
        ...
        // Decreased indentation - emit DEDENT(s)
            ...
            if (this.indentStack[this.indentStack.length - 1] !== indent) {
                this.error('Indentation error - misaligned dedent', this.line, this.column);
            }
        }
    ...
    readString() {
        ...
        if (this.peek() !== quote) {
            this.error('Unterminated string', this.line, startCol);
        }
    ...
    readColorLiteral() {
        ...
        if (value.length !== 7 && value.length !== 9) {
            this.error(`Invalid color literal '${value}'`, this.line, startCol);
        }
