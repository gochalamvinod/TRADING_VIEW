...
### R1. Live Real-Time Continuous Tick Streaming & Weekend Heartbeat Engine
- When MetaTrader 5 terminal is not emitting fresh ticks (during weekends, market closures, or quiet symbol intervals), the system must automatically stream realistic synthetic micro-ticks (bid/ask spread preserved, micro point jitter, live current UTC timestamp) at 200–500ms intervals over WebSocket `/ws/quotes` and in-memory ring buffers for all monitored and subscribed symbols (BTCUSD, XAUUSD., EURUSD., etc.).
- When real MT5 ticks arrive, real ticks must take precedence with 0ms delay.
- The chart must be actively moving in real time at all times: line chart advances, `1T` plots points on every tick, and seconds countdown timers run smoothly.

### R2. Deterministic Broker Timezone Offset & Zero-Drift Server Time
- Fix the dynamic broker timezone offset calculation in `broker_time.py` and `hft_engine.py`. Eliminate the bug where market staleness (`tick.time - now`) was treated as timezone difference, which erroneously shifted timestamps by -18000s (5 hours).
- Calculate broker timezone offset deterministically using MT5 D1 bar timestamp alignment (`rate['time'] % 86400`, which is 0 for Orbex Global) or from live ticks arriving within the last 5 seconds.
- Ensure `/time` returns the exact high-resolution UTC server time matching MT5 and TradingView expectations.

### R3. Robust Seconds & Tick Resolution History Fetching
- In `server.py` (`/history`), replace naive `datetime.fromtimestamp(...)` calls with integer UNIX timestamps `int(from_val)` and `int(to_val)` for `mt5.copy_ticks_range` and `mt5.copy_ticks_from`.
- For `1S`, `5S`, `10S`, `15S` and `1T`, `10T`, `40T`, if the requested window falls during market closure, cleanly retrieve the most recent ticks up to the last available tick and format them without negative filtering or future time corruption.
- Ensure seconds bucketing and 1T tick formatting produce strictly monotonic UTC timestamps without artificial time jumps or cliff drops.

### R4. Dual-Port Tri-Service Integration & Automated CDP Verification
- Ensure both Website (`http://localhost:9000`) and Proxy Site (`http://127.0.0.1:9999`) operate concurrently with the Python backend (`http://127.0.0.1:8080`).
- Validate end-to-end in headless Chrome using Chrome DevTools Protocol (CDP):
  - Verify `BTCUSD` and `XAUUSD.` stream live ticks and advance the chart.
  - Verify `1S` and `5S` form and close real-time candles.
  - Verify `1T` plots live points without flatlines or freezing.
  - Verify Account Center / Account Manager loads with 0 console errors and 0 exceptions.

## Acceptance Criteria

### Live Chart Movement
- [ ] On `BTCUSD` and `XAUUSD.`, the chart line and candle advance live in real-time without flatlines or freezing, even when MT5 market is closed.
- [ ] On `1T` resolution, new tick points are plotted continuously.
- [ ] On `1S` and `5S` resolutions, seconds candles open and close dynamically.

### Time Synchronization
- [ ] Chart timestamp axis aligns with the TradingView bottom-right UTC clock (no 5-hour future shift, no cliff drop).
- [ ] `/time` endpoint returns accurate current UTC timestamp.

### Account Center & System Health
- [ ] Bottom Account Center displays balance, equity, and open positions without infinite spinners.
- [ ] Dual ports 9000 and 9999 both respond with 200 OK and 0 browser console errors.

## 2026-09-12T05:11:28Z

Use a very large team of agents with a Binary Tree exploration structure. Requested team: Advanced Testers, Developers, and a Binary Tree Agent to dig all possibilities to find 99.999% of every bug across UI buttons, data feeds, resolution switching, timing synchronization, and high-frequency streaming speed targeting <1ms or nearest.
...
### R1. Binary Tree Systematic Bug Exploration (99.999% Possibility Exhaustion)
- The Binary Tree Agent and Tester Team must exhaustively traverse every possible state transition, UI button, navigation path, and data scenario:
  - **Resolution switches**: 1S, 5S, 10S, 15S, 30S, 1, 3, 5, 15, 30, 45, 1H, 2H, 3H, 4H, 1D, 1W, 1M, and tick resolutions (1T, 10T, 40T, 100T).
  - **Interactive Controls & Buttons**: Account Center tabs (Positions, Orders, History, Account Summary), Symbol search & quick switcher, Chart type toggles (Bars, Candles, Hollow Candles, Heikin Ashi, Line, Area, Baseline), Timeframe shortcuts, Indicator menu, Timezone dropdowns.
  - **Error & Edge States**: Empty responses, market closed/weekend conditions, rapid symbol changing, reconnecting WebSockets, network latency spikes, rapid clicking, and concurrent requests.

### R2. Ultra-Low Latency Streaming Pipeline & Weekend BTCUSD Live Execution (Target Speed < 1ms)
- Automatically fetch live market open/close status from MetaTrader 5 terminal via `/market_status`:
  - Detect weekend market closures across Forex and Metals (e.g. `XAUUSD.`, `EURUSD.` closed on Saturday/Sunday).
  - Focus active testing and live trading on `BTCUSD`: since cryptocurrency operates 24/7, stream realistic synthetic micro-ticks (bid/ask spread preserved, micro-point walk, UTC millisecond precision) with 0ms buffering delay.
  - Optimize ring buffer lookups, tick serialization, and broadcast handling in `hft_engine.py` and `frontend_server.js` to guarantee < 1ms latency.
  - Keep the chart line, countdown timers, and tick charts (`1T`, `1S`, `5S`) actively advancing in real time.

### R3. Flawless Zero-Drift Server Time & Accurate Axis Alignment
- Guarantee strict UTC time synchronization across MT5 bridge, FastAPI backend (`/time`), Node.js proxy (`/time`), and frontend `ServerTimeSyncEngine` (`index.html`).
- Completely eliminate any artificial time drift, timezone corruption, or cliff-drop plotting (no 5-hour jumps, no flatlines).
- TradingView bottom-right UTC timescale clock, bar timestamps, and candle countdowns must match true UTC with millisecond precision.

### R4. Complete Tri-Service Dual-Port Integration & Automated CDP Verification
- Ensure seamless operation across Website (`http://localhost:9000`), Proxy Site (`http://127.0.0.1:9999`), and Python Backend (`http://127.0.0.1:8080`).
- Deploy automated Chrome DevTools Protocol (CDP) test suites:
  - Verify live continuous chart movement on `BTCUSD` and `XAUUSD.` at `1T`, `1S`, `5S`, and `1D`.
  - Verify Account Center displays live balances, equity, and tables without console errors or endless spinners.
  - Verify all top toolbar buttons, interval selectors, and symbol search dialogues respond instantaneously without runtime exceptions.

## Acceptance Criteria

### Binary Tree Exhaustive Bug Coverage
- [ ] Every tested button, dropdown, modal, and resolution switch transitions cleanly without unhandled JavaScript exceptions or console errors.
- [ ] No dead clicks, broken handlers, or hanging network promises across both `localhost:9000` and `127.0.0.1:9999`.

### Live Chart Movement & Latency (<1ms Target)
- [ ] Live ticks stream continuously 24/7 (including weekends/closed markets) via micro-tick heartbeat; charts never freeze or flatline.
- [ ] On `1T` resolution, tick points advance continuously without cliff drops.
- [ ] Ingestion-to-broadcast processing latency benchmarked at <1ms or nearest achievable.

### Time Synchronization
- [ ] Server time endpoints (`/time`) on both port 8080 and port 9999/9000 return consistent UTC timestamps.
- [ ] Chart timestamp axis aligns seamlessly with the bottom-right UTC clock.

### Account Center & System Health
- [ ] Bottom Account Center panel loads balance, equity, and order tabs with zero console errors.
- [ ] Tri-service architecture (ports 9000, 9999, 8080) runs stably with automated restart recovery.

## 2026-09-13T05:42:15Z
...
### R1. Interactive Bar Replay System (from gaozhao7/tradingview-library)
- Integrate native scissors bar selection via TradingView's `widget.activeChart().requestSelectBar()` triggered from a dedicated "Replay" header toolbar button.
- Provide authentic header replay controls:
...
- Enhance the UDF Datafeed to slice history at the selected cutoff timestamp, buffer future candles, and stream them sequentially to `onRealtimeCallback`.
- Incorporate the complete LocalStorage chart and drawing template save/load adapter.

### R2. 95% Frontend Offloading to Node.js Backend
- Remove client-side `PineTS-main/dist/pinets.min.browser.js` (606 KB) from `index.html` to minimize initial bundle size and parse time.
- Offload 100% of Pine Script transpilation, AST generation, syntax error diagnostics, and indicator descriptor creation to the pre-warmed in-memory Node.js backend via `POST /pine/transpile` and `POST /pine/compile`.
- Refactor `pine_indicators.js` to eliminate the embedded duplicate compiler bundle, leaving only lightweight TradingView study registration and shape dispatchers.
...
### Bar Replay Verification
- [ ] Clicking "Replay" in the chart toolbar displays the scissors cursor.
- [ ] Clicking any historical bar cuts the chart series at that bar and reveals Play, Step, Speed, and Exit controls.
- [ ] Clicking "Play" streams future candles sequentially at the selected speed.
- [ ] Clicking "Step" advances exactly 1 candle.
- [ ] Clicking "Exit" restores real-time MT5 tick streaming.

### Frontend Lightening Verification
- [ ] `index.html` does not load `pinets.min.browser.js`.
- [ ] Total client script payload reduced by >600 KB.
- [ ] Pine Editor compiles custom scripts via backend Node.js `POST /pine/transpile` in < 5ms.
- [ ] Adding custom Pine Script indicators to the chart functions without client-side parsing libraries.
