...
master_features = set(feat_data['master_features'])
...
# The remaining features naturally fall into Watchlist & Tools (watchlists, widgets, symbol search, drawings, studies, storage, dialogs)
WATCHLIST_TOOLS_FEATURES = master_features - TRADING_FEATURES - DATAFEED_SCALES_FEATURES - CHARTS_STYLES_FEATURES

print(f"Trading & Execution: {len(TRADING_FEATURES)}")
print(f"Datafeed & Scales: {len(DATAFEED_SCALES_FEATURES)}")
print(f"Charts & Styles: {len(CHARTS_STYLES_FEATURES)}")
print(f"Watchlist & Tools: {len(WATCHLIST_TOOLS_FEATURES)}")
total_assigned = len(TRADING_FEATURES) + len(DATAFEED_SCALES_FEATURES) + len(CHARTS_STYLES_FEATURES) + len(WATCHLIST_TOOLS_FEATURES)
print(f"Total classified: {total_assigned} (Target: {len(master_features)})")
assert total_assigned == len(master_features), "Mismatch in total classified features!"

# Check intersection
assert len(TRADING_FEATURES & DATAFEED_SCALES_FEATURES) == 0
assert len(TRADING_FEATURES & CHARTS_STYLES_FEATURES) == 0
assert len(TRADING_FEATURES & WATCHLIST_TOOLS_FEATURES) == 0
assert len(DATAFEED_SCALES_FEATURES & CHARTS_STYLES_FEATURES) == 0
assert len(DATAFEED_SCALES_FEATURES & WATCHLIST_TOOLS_FEATURES) == 0
assert len(CHARTS_STYLES_FEATURES & WATCHLIST_TOOLS_FEATURES) == 0

print("All 299 features cleanly partitioned into 4 disjoint categories!")
