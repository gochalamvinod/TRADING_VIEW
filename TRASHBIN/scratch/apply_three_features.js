...
console.log('--- Applying 3 Requested Features ---');
const idePath = 'e:/TRADINGVIEW ADVANCED/pine_editor_ide.js';
let code = fs.readFileSync(idePath, 'utf8');

// 1. Ensure mountAlertsPanelAndModal is called at startup and in openCreateAlertDialog
if (!code.includes('mountAlertsPanelAndModal();\n    const modalBackdrop = document.getElementById(\'tv_alert_create_backdrop\');')) {
  ...
  console.log('Updated openCreateAlertDialog to mount alerts modal');
}

// 2. Add Built-in Pine Script generator helper
const builtinHelper = `
...
if (!code.includes('function getBuiltinScriptCode')) {
  ...
  console.log('Inserted getBuiltinScriptCode, openScriptForStudy, and showReadOnlyCopyPrompt');
}

// 3. Update loadScript implementation to respect isReadOnly
...
if (code.includes(oldLoadScriptImpl)) {
  ...
  console.log('Updated loadScript to properly set and display isReadOnly');
}

// 4. Update codeInput keydown to enforce read-only copy condition
const oldKeydownStart = `    codeInput.addEventListener('keydown', (e) => {`;
const newKeydownStart = `    codeInput.addEventListener('keydown', (e) => {
...
if (!code.includes('if (_currentScript.isReadOnly) {') && code.includes(oldKeydownStart)) {
  code = code.replace(oldKeydownStart, newKeydownStart);
  console.log('Enforced read-only copy prompt on typing in built-in script');
}

// 5. Update saveBtn action to prompt for copy if read-only
...
if (!code.includes('if (_currentScript.isReadOnly) {\n          showReadOnlyCopyPrompt();') && code.includes(oldSaveBtn)) {
  ...
  console.log('Enforced copy prompt on save button for read-only script');
}

// 6. Update Indicators Modal action buttons: clean { } button for BOTH custom and built-in
...
if (code.includes(oldRowHtml)) {
  ...
  console.log('Cleaned { } button in Indicators modal');
}
...
if (code.includes(oldEditorClick)) {
  ...
  console.log('Updated open-editor-btn click to use openScriptForStudy');
}

// 7. Inject syncLegendCodeButtons function to inject { } button into study items in chart iframe legend
...
if (!code.includes('function syncLegendCodeButtons')) {
  ...
  console.log('Inserted syncLegendCodeButtons');
}

// 8. Call syncLegendCodeButtons in hookChartIframeIndicatorButtons and periodically
if (!code.includes('setInterval(syncLegendCodeButtons, 1500)')) {
  ...
  console.log('Hooked periodic legend code buttons sync');
}

// 9. Add Alert button click hook in hookChartIframeIndicatorButtons
const alertHookCode = `
...
if (!code.includes('[data-name="alerts"], [data-name="alert"]')) {
  ...
  console.log('Added Alerts button click handler to iframe click listener');
}

// 10. Export openScriptForStudy
if (!code.includes('openScriptForStudy,')) {
  ...
  console.log('Exported openScriptForStudy to PineEditorIDE and root');
}

// Also ensure mountAlertsPanelAndModal is called when DOM is ready
if (!code.includes('mountAlertsPanelAndModal();')) {
  code = code.replace('mountIndicatorsModal();', 'mountIndicatorsModal();\n      mountAlertsPanelAndModal();');
  console.log('Added mountAlertsPanelAndModal call to DOM init');
}

fs.writeFileSync(idePath, code, 'utf8');
console.log('pine_editor_ide.js updated with all 3 features successfully!');
