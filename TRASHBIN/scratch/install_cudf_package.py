...
class Series:
    def __init__(self, data, name: Optional[str] = None, dtype=None):
        ...
        if isinstance(data, Series):
            ...
            self._data = cp.asarray(data, dtype=cp.float32 if dtype is None else dtype)
...
# 1. Install to Python site-packages
sp = r'C:\Users\gocha\AppData\Local\Programs\Python\Python311\Lib\site-packages'
cudf_pkg_dir = os.path.join(sp, 'cudf')
os.makedirs(cudf_pkg_dir, exist_ok=True)
with open(os.path.join(cudf_pkg_dir, '__init__.py'), 'w', encoding='utf-8') as f:
    f.write(cudf_code)
print(f"Installed cudf to {cudf_pkg_dir}")

# 2. Also install into workspace root
ws_dir = r'e:\TRADINGVIEW ADVANCED\cudf'
os.makedirs(ws_dir, exist_ok=True)
with open(os.path.join(ws_dir, '__init__.py'), 'w', encoding='utf-8') as f:
    f.write(cudf_code)
print(f"Installed cudf to {ws_dir}")
