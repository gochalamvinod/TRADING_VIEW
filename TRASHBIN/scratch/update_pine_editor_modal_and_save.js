...
let code = fs.readFileSync(pineIdePath, 'utf8');

// 1. Normalize line endings for reliable replacement
const isCRLF = code.includes('\r\n');
const lineEnding = isCRLF ? '\r\n' : '\n';

// 2. The 107 authentic TradingView built-in indicators
...
// Add BUILTIN_TECHNICALS array if not present
if (!code.includes('const BUILTIN_TECHNICALS =')) {
  const target = 'const FAVORITE_BUILTINS_KEY = "tv_favorite_builtins";';
  code = code.replace(target, target + lineEnding + '  const BUILTIN_TECHNICALS = ' + JSON.stringify(BUILTIN_TECHNICALS, null, 2) + ';' + lineEnding);
  console.log('1. Added BUILTIN_TECHNICALS.');
}

// 3. Mount cleanup: preserve legitimate user scripts
const oldCleanPattern = /const cleaned = parsed\.filter\([\s\S]*?localStorage\.setItem\(USER_SAVED_SCRIPTS_KEY, JSON\.stringify\(cleaned\)\);/;
const newCleanCode = `const cleaned = parsed.filter(s => {
            const n = String(s?.name || '').toLowerCase();
            return !n.includes('vinod') && n !== 'sessions [luxalgo]';
          });
          localStorage.setItem(USER_SAVED_SCRIPTS_KEY, JSON.stringify(cleaned));`;

if (oldCleanPattern.test(code)) {
  code = code.replace(oldCleanPattern, newCleanCode);
  console.log('2. Preserved user scripts in mount cleanup.');
}

// 4. Update saveBtn in bindEvents to sync with User Saved Scripts
const oldSaveBtnPattern = /\/\/\s*Save Action[\r\n\s]*if\s*\(saveBtn\)\s*\{[\s\S]*?logConsole\(`Script "\${_currentScript\.name}" saved successfully\.`, "success"\);[\s\S]*?await runCompilation\(false\);[\r\n\s]*\};?\s*\}/;

const newSaveBtnCode = `// Save Action (Ctrl+S / Save script)
    if (saveBtn) {
...
if (oldSaveBtnPattern.test(code)) {
  code = code.replace(oldSaveBtnPattern, newSaveBtnCode);
  console.log('3. Updated saveBtn to sync with User Saved Scripts.');
} else {
  console.error('Could not match oldSaveBtnPattern');
}

// 5. Update renderIndicatorsModal to support Technicals (107 built-ins), My scripts, Favorites
const oldRenderPattern = /function renderIndicatorsModal\(\) \{[\s\S]*?list\.appendChild\(row\);\s*\}\);\s*\}/;

const newRenderCode = `function renderIndicatorsModal() {
...
if (oldRenderPattern.test(code)) {
  code = code.replace(oldRenderPattern, newRenderCode);
  console.log('4. Updated renderIndicatorsModal.');
} else {
  console.error('Could not match oldRenderPattern');
}

// 6. Clean mountIndicatorsModal sidebar HTML: STRICTLY Favorites, My scripts, Technicals
const oldModalBodyPattern = /<div class="tv-indicators-sidebar">[\s\S]*?<\/div>\s*<div class="tv-indicators-content">/;
...
if (oldModalBodyPattern.test(code)) {
  code = code.replace(oldModalBodyPattern, newModalBody);
  console.log('5. Cleaned modal sidebar to strictly Favorites, My scripts, Technicals.');
} else {
  console.error('Could not match oldModalBodyPattern');
}

fs.writeFileSync(pineIdePath, code, 'utf8');
console.log('Updated pine_editor_ide.js successfully.');
