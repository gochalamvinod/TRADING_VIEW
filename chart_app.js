/* =========================================================================
      ...
      async function initChart() {
        ...
        const datafeed = new Datafeeds.UDFCompatibleDatafeed(datafeedUrl, 250);
        ...
        async function syncReplayFutureBars(targetSymbol, targetResolution) {
          ...
          // Abort any prior in-flight fetch immediately
          if (_replayAbortController) {
            try { _replayAbortController.abort(); } catch(_) {}
          }
          _replayAbortController = new AbortController();
          const signal = _replayAbortController.signal;
          ...
          const sym = targetSymbol || (sub && sub.symbol) || (chart && typeof chart.symbol === 'function' ? chart.symbol() : "");
          const res = String(targetResolution || (sub && sub.resolution) || (chart && typeof chart.resolution === 'function' ? chart.resolution() : "1"));
          if (!sym) {
            console.warn("[BarReplay] No active chart symbol detected for future bars sync.");
          ...
          try {
            ...
            // Calculate bar span in seconds to cap lookahead (bounded to 5,000 bars max)
            let resSec = 60;
            const rStr = res.toUpperCase();
            if (rStr.endsWith('S')) resSec = Math.max(1, parseInt(rStr.slice(0, -1), 10) || 1);
            else if (rStr === 'D' || rStr === '1D') resSec = 86400;
            else if (rStr === 'W' || rStr === '1W') resSec = 604800;
            else if (rStr === 'M' || rStr === '1M') resSec = 2592000;
            else if (!rStr.endsWith('T') && rStr !== 'T') resSec = Math.max(1, parseInt(rStr, 10) || 1) * 60;

            const maxFutureSpan = Math.min(86400 * 365, 5000 * resSec);
            const nowSec = Math.floor(Date.now() / 1000) + 86400;
            const toSec = Math.min(nowSec, fromSec + maxFutureSpan);

            let historyUrl = `${datafeedUrl}/history?symbol=${encodeURIComponent(sym)}&resolution=${encodeURIComponent(res)}&from=${fromSec}&to=${toSec}`;
            if (res.endsWith("S") || res.endsWith("T") || res === "T") {
              historyUrl += `&countback=5000`;
            }

            const resp = await fetch(historyUrl, { signal });
            if (seq !== _replaySyncSeq || signal.aborted) return;

            if (resp.ok) {
              const data = await resp.json();
              if (seq !== _replaySyncSeq || signal.aborted) return; // Guard post-parse race

              if (data.s === 'ok' && Array.isArray(data.t)) {
                const fBars = [];
                let prevTime = 0;
                const maxBars = 10000;
                for (let i = 0; i < data.t.length && fBars.length < maxBars; i++) {
            ...
            if (seq === _replaySyncSeq) {
              BAR_REPLAY.isBuffering = false;
              updateReplayToolbarUI();
              if (wasPlaying && BAR_REPLAY.active && BAR_REPLAY.status !== 'paused' && BAR_REPLAY.futureBars.length > 0) {
                barReplayPlay();
              }
            }
          }
        ...
        async function enterReplayMode(cutoffTime, targetSym, targetRes) {
          // 1. Turn off WebSockets and live pollers FIRST (User Requirement)
          if (typeof disconnectQuoteWebSocket === "function") {
            disconnectQuoteWebSocket();
          }
          if (historyPollTimer) {
            clearInterval(historyPollTimer);
            historyPollTimer = null;
          }
          // Take time to settle and complete formalities
          ...
          const chart = widget && typeof widget.activeChart === 'function' ? widget.activeChart() : null;
          const sub = activeBarSubscribers.values().next().value;
          const sym = targetSym || (sub && sub.symbol) || (chart && typeof chart.symbol === 'function' ? chart.symbol() : "");
          const res = String(targetRes || (sub && sub.resolution) || (chart && typeof chart.resolution === 'function' ? chart.resolution() : "1"));
          if (!sym) {
            console.warn("[BarReplay] No active chart symbol detected for replay mode entry.");
            return;
          }
        ...
        function exitReplayMode() {
          ...
          // 2. Turn on WebSockets and resume real-time tick streaming (User Requirement)
          if (typeof connectQuoteWebSocket === "function") {
            connectQuoteWebSocket();
          }
          if (typeof startHistoryPolling === "function" && adaptiveConfig) {
            startHistoryPolling(adaptiveConfig.barPollIntervalMs || 250);
          }
        }
        ...
        datafeed.getBars = function(symbolInfo, resolution, periodParams, onHistoryCallback, onErrorCallback) {
          // If in Bar Replay mode, strictly clamp periodParams.to and periodParams.from to BAR_REPLAY.cutoffSec!
          if (window.BAR_REPLAY && window.BAR_REPLAY.active && window.BAR_REPLAY.cutoffSec) {
            const cutoffSec = window.BAR_REPLAY.cutoffSec;
            if (periodParams.firstDataRequest || periodParams.to > cutoffSec) {
              periodParams.to = cutoffSec;
            }
            // Parse resolution to determine duration in seconds
            let resSec = 60;
            const rStr = String(resolution || '1').trim().toUpperCase();
            if (rStr.endsWith('S')) {
              const sVal = parseInt(rStr.slice(0, -1), 10);
              resSec = isNaN(sVal) || sVal <= 0 ? 1 : sVal;
            } else if (rStr.endsWith('T') || rStr === 'T') {
              resSec = 1;
            } else if (rStr === 'D' || rStr === '1D') {
              resSec = 86400;
            } else if (rStr === 'W' || rStr === '1W') {
              resSec = 86400 * 7;
            } else if (rStr === 'M' || rStr === '1M') {
              resSec = 86400 * 30;
            } else {
              const mVal = parseInt(rStr, 10);
              resSec = isNaN(mVal) || mVal <= 0 ? 60 : mVal * 60;
            }
            const cb = Math.max(periodParams.countBack || 300, 300);
            if (!periodParams.from || periodParams.from >= periodParams.to) {
              periodParams.from = periodParams.to - (cb * resSec);
            }
            if (periodParams.from >= periodParams.to) {
              periodParams.from = periodParams.to - 300;
            }
          }
        ...
        datafeed.subscribeBars = function(symbolInfo, resolution, onRealtimeCallback, listenerGUID, onResetCacheNeededCallback) {
          ...
          const monotonicRealtimeCallback = function(bar) {
            ...
            if (bar.time < subObj.lastDispatchedTime && !(window.BAR_REPLAY && window.BAR_REPLAY.active)) {
              return; // reject out-of-order bars to eliminate feed time violations
            }
            subObj.lastDispatchedTime = bar.time;
            onRealtimeCallback(bar);
          };
          subObj.onRealtimeCallback = monotonicRealtimeCallback;

          activeBarSubscribers.set(listenerGUID, subObj);
        ...
        // ── WebSocket Quote & Tick Client (Real-Time 1ms Stream) ──────────────
        ...
        function connectQuoteWebSocket() {
          ...
          if (quoteWs && (quoteWs.readyState === WebSocket.OPEN || quoteWs.readyState === WebSocket.CONNECTING)) {
            return;
          }
          try {
            wsConnecting = true;
            ...
            quoteWs.onopen = function() {
              ...
              if (allSyms.size === 0) {
                ["XAUUSD.", "EURUSD.", "GBPUSD.", "USDJPY.", "BTCUSD.", "XAGUSD."].forEach(s => allSyms.add(s));
              }
              sendWsSubscriptions(Array.from(allSyms));
            };

            quoteWs.onmessage = function(event) {
              if (window.BAR_REPLAY && window.BAR_REPLAY.active) return; // Strict guard: reject during replay
              try {
                ...
                if (msg.type === "quote" && msg.data) {
                  ...
                  // 3. DIRECT 1MS REAL-TIME BAR STREAMING (Zero Buffering Delay)
                  const v = qData.v || {};
            ...
            quoteWs.onclose = function() {
              ...
              wsConnecting = false;
              if (!(window.BAR_REPLAY && window.BAR_REPLAY.active)) {
                const delay = adaptiveConfig ? adaptiveConfig.wsReconnectMs : 500;
                if (!wsReconnectTimer) {
        ...
        function disconnectQuoteWebSocket() {
          if (wsReconnectTimer) {
            clearTimeout(wsReconnectTimer);
            wsReconnectTimer = null;
          }
          if (quoteWs) {
            try {
              quoteWs.onclose = null;
              quoteWs.onerror = null;
              quoteWs.close();
            } catch(e) {}
            quoteWs = null;
          }
        ...
        window.connectQuoteWebSocket = connectQuoteWebSocket;
        window.disconnectQuoteWebSocket = disconnectQuoteWebSocket;

        function sendWsSubscriptions(symbols) {
          if (!quoteWs || quoteWs.readyState !== WebSocket.OPEN) {
            connectQuoteWebSocket();
            return;
          }
        ...
        // ── Real-Time /history Poller (Fast Fallback when WS is offline) ─────
        let historyPollTimer = null;
        let isHistoryPollInFlight = false;

        async function pollHistoryRealtime() {
          if (window.BAR_REPLAY && window.BAR_REPLAY.active) return;
          if (quoteWs && quoteWs.readyState === WebSocket.OPEN) return; // WS is active, skip HTTP polling
          if (activeBarSubscribers.size === 0) return;
          if (isHistoryPollInFlight) return;

          isHistoryPollInFlight = true;
          try {
            const nowSec = Math.floor(Date.now() / 1000);

            for (const sub of activeBarSubscribers.values()) {
              const resMs = sub.resolutionMs || getResolutionInMs(sub.resolution);
              const lookbackSec = Math.max(300, Math.ceil((resMs * 4) / 1000));
              const fromSec = nowSec - lookbackSec;
              const toSec = nowSec + 60;

              const url = `${datafeedUrl}/history?symbol=${encodeURIComponent(sub.symbol)}&resolution=${encodeURIComponent(sub.resolution)}&from=${fromSec}&to=${toSec}&countback=2`;
              const res = await fetch(url);
              if (!res.ok) continue;
              const data = await res.json();
              if (data && data.s === "ok" && Array.isArray(data.t) && data.t.length > 0) {
                const idx = data.t.length - 1;
                const rawT = data.t[idx];
                const barTimeMs = (rawT < 1e11) ? Math.round(rawT * 1000) : Math.round(rawT);
                const latestBar = {
                  time: barTimeMs,
                  ...
                  volume: Number(data.v ? data.v[idx] : 1) || 1
                };

                if (typeof window.evaluateAlertsForPrice === 'function') {
                  window.evaluateAlertsForPrice(sub.cleanSymbol, latestBar.close);
                }

                sub.currentBar = latestBar;
                window._activeOpenCandle = latestBar;
                sub.onRealtimeCallback({ ...latestBar });

                // Dispatch quote update to active quote listeners
                const qData = {
                  s: "ok",
                  n: sub.symbol,
                  v: {
                    lp: latestBar.close,
                    last_price: latestBar.close,
                    bid: latestBar.close,
                    ask: latestBar.close,
                    volume: latestBar.volume,
                    time: Math.floor(latestBar.time / 1000)
                  }
                };
                for (const qSub of activeQuoteListeners.values()) {
                  if (qSub.symbols.includes(sub.symbol) || qSub.symbols.includes(sub.cleanSymbol)) {
                    qSub.callback([qData]);
                  }
                }
              }
            ...
            isHistoryPollInFlight = false;
          }
        }

        function startHistoryPolling(intervalMs) {
          if (historyPollTimer) clearInterval(historyPollTimer);
          const safeInterval = Math.max(100, intervalMs || 250);
          historyPollTimer = setInterval(pollHistoryRealtime, safeInterval);
        }
        ...
        function startAdaptiveQuotePoller(intervalMs) {
          ...
          const safeInterval = Math.max(intervalMs || 500, 250);
          quotePollTimer = setInterval(executePollFallback, safeInterval);
        }

        // ── Adaptive Bar Pulse Provider (/history Polling) ───────────────────
        function startAdaptiveBarPulse(intervalMs) {
          if (barPulseTimer) clearInterval(barPulseTimer);
          barPulseTimer = setInterval(() => {
            if (datafeed && datafeed._dataPulseProvider && typeof datafeed._dataPulseProvider._updateData === "function") {
        ...
        function updateAdaptiveTimeframe(newResolution) {
          ...
          startHistoryPolling(adaptiveConfig.barPollIntervalMs);
