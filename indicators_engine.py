...
def compute_indicator(
    ...
    j_c = jnp.asarray(c_series.to_numpy())
    j_h = jnp.asarray(h_series.to_numpy())
    j_l = jnp.asarray(l_series.to_numpy())
    j_v = jnp.asarray(v_series.to_numpy())
    ...
    if name_upper in ("SMA", "MOVING AVERAGE", "MOVING AVERAGE SIMPLE"):
        ...
        length = max(1, int(params.get("length", 14)))
        plots["plot_0"] = _jax_ema(j_c, length).tolist()
