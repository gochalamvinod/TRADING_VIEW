...
@app.get("/device")
def get_device_info():
    """Return runtime hardware specs, CPU cores, RAM, GPU status, and calibrated parameters."""
    from dataclasses import asdict
    from hardware_profile import current_profile
    return {"s": "ok", "device": asdict(current_profile())}


from collections import OrderedDict

_HISTORY_CACHE_MAX_SIZE = 256
_HISTORY_CACHE_TTL_SEC = 5.0
_history_cache: OrderedDict = OrderedDict()

def _get_history_cache(key):
    cached = _history_cache.get(key)
    if cached is not None:
        cached_ts, payload = cached
        if (time.time() - cached_ts) < _HISTORY_CACHE_TTL_SEC:
...
def _set_history_cache(key, payload: bytes):
    _history_cache[key] = (time.time(), payload)
    _history_cache.move_to_end(key)
    while len(_history_cache) > _HISTORY_CACHE_MAX_SIZE:
...
async def get_history(
    ...
    # Sub-millisecond Active Bar Streaming Fast-Path
    if countback is not None and countback <= 2:
        try:
            import c_bridge
            fast_bytes = c_bridge.active_bar_get_json(f"{resolved_symbol}:{res}")
            if fast_bytes is not None:
                return Response(content=fast_bytes, media_type="application/json")
        except Exception:
            pass
        fast_bytes = hft_engine.get_active_bar_bytes(resolved_symbol, res, countback)
        if fast_bytes is not None:
            return Response(content=fast_bytes, media_type="application/json")

    # In-memory warm cache for rapid consecutive polling
    cache_key = (resolved_symbol, res, _from, to, countback)
    cached_payload = _get_history_cache(cache_key)
    ...
    if res.endswith("S"):
        ...
        try:
            ...
            cb = countback or 300
            req_span = max(300.0, float(cb * sec * 2))
            if _from is not None and float(_from) > 0:
                from_val = float(_from)
                if from_val >= to_val:
                    from_val = max(0.0, to_val - req_span)
            else:
                from_val = max(0.0, to_val - req_span)
            ...
            if (raw_ticks is None or len(raw_ticks) == 0) and to_val >= (now_utc - 60):
                ...
                if last_tick and last_tick.time > 0:
                    ...
                    if raw_ticks is None or len(raw_ticks) == 0:
                        raw_ticks = mt5.copy_ticks_from(resolved_symbol, start_b, int(cb * 50), mt5.COPY_TICKS_ALL)

            if raw_ticks is None or len(raw_ticks) == 0:
                records = {"s": "no_data", "t": [], "o": [], "h": [], "l": [], "c": [], "v": []}
            else:
                has_msc = 'time_msc' in raw_ticks.dtype.names
                raw_msc = raw_ticks['time_msc'] if has_msc else (raw_ticks['time'] * 1000).astype(np.int64)
                t_utc_sec = ((raw_msc - int(offset * 1000)) // 1000).astype(np.int64)
                if PRICE_TYPE == "ASK":
                    prices = cp.asarray(raw_ticks['ask'], dtype=cp.float64)
                elif PRICE_TYPE == "MID":
                    _b = cp.asarray(raw_ticks['bid'], dtype=cp.float64)
                    _a = cp.asarray(raw_ticks['ask'], dtype=cp.float64)
                    prices = cp.where((_b > 0) & (_a > 0), (_b + _a) * 0.5, cp.where(_b > 0, _b, _a))
                else:
                    prices = cp.asarray(raw_ticks['bid'], dtype=cp.float64)
                vols = cp.asarray(raw_ticks['volume_real'] if 'volume_real' in raw_ticks.dtype.names else raw_ticks['volume'], dtype=cp.float64)

                import c_bridge
                res_c = c_bridge.resample_seconds(t_utc_sec, prices, vols, sec) if c_bridge.is_available() else None
                if res_c is not None and res_c.get("s") == "ok":
                    t_list = res_c["t"].tolist() if hasattr(res_c["t"], "tolist") else list(res_c["t"])
                    o_list = res_c["o"].tolist() if hasattr(res_c["o"], "tolist") else list(res_c["o"])
                    h_list = res_c["h"].tolist() if hasattr(res_c["h"], "tolist") else list(res_c["h"])
                    l_list = res_c["l"].tolist() if hasattr(res_c["l"], "tolist") else list(res_c["l"])
                    c_list = res_c["c"].tolist() if hasattr(res_c["c"], "tolist") else list(res_c["c"])
                    v_list = res_c["v"].tolist() if hasattr(res_c["v"], "tolist") else list(res_c["v"])
                    if countback and len(t_list) > countback:
                        t_list = t_list[-countback:]
                        o_list = o_list[-countback:]
                        h_list = h_list[-countback:]
                        l_list = l_list[-countback:]
                        c_list = c_list[-countback:]
                        v_list = v_list[-countback:]
                    records = {"s": "ok", "t": t_list, "o": o_list, "h": h_list, "l": l_list, "c": c_list, "v": v_list}
                else:
                    t_buckets = (cp.asarray(t_utc_sec) // sec) * sec
                    unique_t, idx_start = cp.unique(t_buckets, return_index=True)
                    records = {
                        "s": "ok",
                        "t": [int(x) for x in cp.asnumpy(unique_t)],
                        "o": [float(prices[i]) for i in cp.asnumpy(idx_start)],
                        "h": [float(prices[i]) for i in cp.asnumpy(idx_start)],
                        "l": [float(prices[i]) for i in cp.asnumpy(idx_start)],
                        "c": [float(prices[i]) for i in cp.asnumpy(idx_start)],
                        "v": [float(vols[i]) for i in cp.asnumpy(idx_start)],
                    }

            # Seed active bar cache
            if records.get("s") == "ok" and len(records.get("t", [])) >= 2:
                try:
                    import c_bridge
                    b_t = records["t"]
                    b_o = records["o"]
                    ...
                    b_c = records["c"]
                    b_v = records["v"]
                    c_bridge.active_bar_seed(
                        f"{resolved_symbol}:{res}", sec,
                        int(b_t[-2]), float(b_o[-2]), float(b_h[-2]), float(b_l[-2]), float(b_c[-2]), float(b_v[-2]),
                        int(b_t[-1]), float(b_o[-1]), float(b_h[-1]), float(b_l[-1]), float(b_c[-1]), float(b_v[-1])
                    )
                    hft_engine.active_bar_mgr.seed_bars(resolved_symbol, res, b_t, b_o, b_h, b_l, b_c, b_v)
                except Exception:
                    pass
            resp_bytes = fast_json_dumps(records)
            _set_history_cache(cache_key, resp_bytes)
            return Response(content=resp_bytes, media_type="application/json")
    ...
    if res.endswith("T") or res == "T":
        ...
        try:
            ...
            t_sub_float = cp.asarray(t_utc_msc / 1000.0, dtype=cp.float64)
            if PRICE_TYPE == "ASK":
                prices = cp.asarray(raw_ticks['ask'], dtype=cp.float64)
            elif PRICE_TYPE == "MID":
                _b = cp.asarray(raw_ticks['bid'], dtype=cp.float64)
            ...
            if num_bars == 0:
                records = {
                    "s": "ok",
                    "t": [round(float(cp.asnumpy(t_sub_float)[0]), 3)],
                    "o": [float(cp.asnumpy(prices)[0])],
                    "h": [float(cp.asnumpy(cp.max(prices)))],
                    "l": [float(cp.asnumpy(cp.min(prices)))],
                    "c": [float(cp.asnumpy(prices)[-1])],
                    "v": [float(cp.asnumpy(cp.sum(vols)))],
                }
            else:
                usable = num_bars * tpb
                ...
                b_o = p_2d[:, 0]
                b_h = cp.max(p_2d, axis=1)
                b_l = cp.min(p_2d, axis=1)
                b_c = p_2d[:, -1]
                b_v = cp.sum(v_2d, axis=1)

                t_raw = cp.round(t_2d[:, -1], 3)
                b_t = []
                last_t = 0.0
                for x in cp.asnumpy(t_raw):
                    cur_t = float(x)
                    if cur_t <= last_t:
                ...
                b_o = [float(x) for x in cp.asnumpy(b_o)]
                b_h = [float(x) for x in cp.asnumpy(b_h)]
                b_l = [float(x) for x in cp.asnumpy(b_l)]
                b_c = [float(x) for x in cp.asnumpy(b_c)]
                b_v = [float(x) for x in cp.asnumpy(b_v)]

                if usable < sub_len:
                    p_rem = prices[usable:]
                    t_host_last = float(cp.asnumpy(t_sub_float)[-1])
                    cur_rem_t = round(t_host_last, 3)
                    if cur_rem_t <= last_t:
                        cur_rem_t = round(last_t + 0.001, 3)
                    b_t.append(cur_rem_t)
                    b_o.append(float(cp.asnumpy(p_rem)[0]))
                    b_h.append(float(cp.asnumpy(cp.max(p_rem))))
                    b_l.append(float(cp.asnumpy(cp.min(p_rem))))
                    b_c.append(float(cp.asnumpy(p_rem)[-1]))
                    b_v.append(float(cp.asnumpy(cp.sum(vols[usable:]))))
                ...
                records = {"s": "ok", "t": b_t, "o": b_o, "h": b_h, "l": b_l, "c": b_c, "v": v_list}
    ...
    if safe_from > safe_to:
        # Client requested data up to safe_to with countback or replay cutoff, but from was defaulted into the future
        # Adjust safe_from backwards from safe_to, NEVER moving safe_to forward into the future!
        safe_span = float(max(countback or 300, 300) * 3600)
        safe_from = max(0.0, safe_to - safe_span)
    ...
    if PRICE_TYPE in ("ASK", "MID"):
        ...
        if hasattr(rates, 'dtype') and 'spread' in rates.dtype.names:
            ...
            if bool((spread_pts == 0).all()) and tick and tick.ask > tick.bid > 0:
    ...
    # Dynamically inject current live tick price ONLY into active forming bar if live request
    is_live_request = (to is None) or (float(to) >= (now_utc - 60))
    if is_live_request and tick and tick.time > 0 and len(t_values) > 0:
